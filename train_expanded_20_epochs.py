#!/usr/bin/env python3
"""
LoRA training with expanded dataset (92 images) and 20 epochs.
Uses mirrored images to double the dataset - aggressive training.
"""
...
print("\n" + "=" * 80)
print("LORA TRAINING - EXPANDED DATASET (92 IMAGES) + 20 EPOCHS (AGGRESSIVE)")
print("=" * 80)
...
try:
    ...
    print("\n[Training Configuration - Expanded Dataset, Aggressive]")
    print(f"  Total Images: {len(image_files)} (46 original + 46 mirrored)")
    print(f"  Epochs: 20 (AGGRESSIVE)")
    print(f"  Batch Size: 1")
    print(f"  LoRA Rank: 64")
    print(f"  Total Steps: {len(image_files) * 20} (~{len(image_files) * 20 // 60} minutes)")
    print()
    print("⚠️ WARNING: This is aggressive training with large dataset.")
    print("   Risk of overfitting, but may yield very style-specific results.")
    print()
    ...
    print("=" * 80)
    print("STARTING AGGRESSIVE TRAINING WITH EXPANDED DATASET (20 EPOCHS)...")
    print("=" * 80 + "\n")
    ...
    result = train_lora(
        ...
        style_token="eudes-correia-expanded-20",
        use_captions=True,
        resolution=768,
        lora_rank=64,
        learning_rate=1e-4,
    ...
    lora_files = list(OUTPUT_DIR.glob("eudes-correia-expanded-20_*.safetensors"))
    if lora_files:
