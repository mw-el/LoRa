#!/usr/bin/env python3
"""
Proof of Concept: Single epoch LoRA training
Tests the LoRA save mechanism with minimal training time.
"""
...
print("\n" + "=" * 80)
print("PROOF OF CONCEPT: SINGLE EPOCH LORA TRAINING")
print("=" * 80)
...
logger.info(f"Starting single-epoch training at {datetime.now()}")
...
try:
    ...
    # Train with 1 epoch only
    print("\n[Training Configuration]")
    print(f"  Epochs: 1 (proof of concept)")
    ...
    result = train_lora(
        ...
        style_token="eudes-correia-poc",
        use_captions=False,
        resolution=768,
        lora_rank=32,
        learning_rate=1e-4,
        num_epochs=1,  # ← SINGLE EPOCH FOR PROOF OF CONCEPT
        batch_size=1,
    ...
    lora_files = list(OUTPUT_DIR.glob("eudes-correia-poc_*.safetensors"))
    if lora_files:
    ...
    print("\n" + "=" * 80)
    print("PROOF OF CONCEPT SUCCESSFUL - LoRA SAVE MECHANISM WORKS!")
    print("=" * 80)
